...
__host__ __device__ static const char *_cudaGetErrorEnum(cudaError_t error)
{
    switch (error)
    {
        case cudaSuccess:
...
#ifndef ENABLE_CRASH_ON_ERROR
#define ENABLE_CRASH_ON_ERROR true
#endif
...
template< typename T >
__device__ void check_device(T result, char const *const func, const char *const file, int const line)
{
	if (result)
	{
...
__host__ __device__ __inline__ void __getLastCudaError(const char *errorMessage, const char *file, const int line)
{
